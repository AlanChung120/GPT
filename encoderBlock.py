...
class EncoderBlock(nn.Module):
  ...
  def __init__(self, headSize, numHeads, nEmbed, blockSize, dropout):
    ...
    self.saHeads = MultiHeadAttention(numHeads, headSize, nEmbed, blockSize, dropout) # (B, T, headSize)
  ...
  def forward(self, x):
    ...
    x = x + self.saHeads(self.layerNorm1(x), False) # (B, T, C/nEmbed/headSize) (residual pathway) + (B, T, headSize) (fork off) = (B, T, headSize)
